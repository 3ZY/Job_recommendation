...
#对user推荐item rank
def itemCF_IUF_Recommend(user,train,W,JOB_OFFER):
	K=100 #最相似的K个职位
	rank=dict()
	...
	for i,pi in interacted_items.items():
		...
		for j,wj in sorted(W[i].items(),key=operator.itemgetter(1),reverse=True)[0:K]:
			if j in interacted_items:#过滤求职者浏览、应聘、收藏过的职位
				continue
			if j not in JOB_OFFER:#非目标职位
				continue
			if j not in rank:
				rank[j]=0
			rank[j]+=pi*wj
...
def itemCF_IUF_finallyRecommend(train,JWINFO,JOB_OFFER,R_Num=10,finallyRecommend={}):
	...
	for u in train.keys():
		...
		if alreadyRecommendNum>=R_Num*3:
			continue
		u_recommend=itemCF_IUF_Recommend(u,train,W,JOB_OFFER) #求职者u的推荐集合
